...
int main(int argc, char* argv[])
{
    ...
    // ---------- 2. 加载纹理 ----------
    TextureInfo tex_info = LoadTextureFromPNG(renderer, "images/results/test.png");
    if (!tex_info.texture) {
    ...
    while (keep_going) {
        ...
        // ---- Step 2: 渲染场景节点 ----
        std::vector<std::pair<KeypointExtraInfos, std::vector<KeypointProjection>>> keypoints_for_render;
        scene.RenderAll(renderer, intrinsics, distortion, camera_pose, keypoints_for_render);
        ...
        if (screenshot_requested) {
            ...
            SaveScreenshot(renderer, "screenshot/test_screenshot");
        }
